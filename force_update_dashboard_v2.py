            ...
            <div class="col-md-6 mb-4">
                <div
                    ...
                    <div class="card-body text-center">
                        ...
                        <form action="{% url 'submit_prediction' item.match.id %}" method="post"
                            class="mt-3 row justify-content-center">
                            {% csrf_token %}
                            <div class="col-3">
                                <input type="number" name="home_score" class="form-control text-center text-dark"
                                    value="{{ item.prediction.home_score|default_if_none:'' }}" {% if item.is_locked
                                    %}disabled{% endif %} required placeholder="casa">
                            </div>
                            <div class="col-auto align-self-center">X</div>
                            <div class="col-3">
                                <input type="number" name="away_score" class="form-control text-center text-dark"
                                    value="{{ item.prediction.away_score|default_if_none:'' }}" {% if item.is_locked
                                    %}disabled{% endif %} required placeholder="fora">
                            </div>
            ...
            <div class="list-group-item d-flex justify-content-between align-items-center">
                <div>
                    <div class="fw-bold d-flex align-items-center">
                        ...
                        {% endif %}
                        {{ item.match.home_team.name }} {{ item.match.home_score }} x {{ item.match.away_score }} {{
                    ...
                    <small class="text-muted">Rodada {{ item.match.round }} - {{ item.match.date|date:"d/m/Y H:i"
                        }}</small>
                </div>
                <div class="text-end">
                    {% if item.prediction %}
