                ...
                <div class="card match-card h-100 {% if item.is_locked %}border-secondary bg-light{% else %}border-primary{% endif %}">
                    ...
                    <div class="card-body text-center">
                        ...
                        <form action="{% url 'submit_prediction' item.match.id %}" method="post" class="mt-3 row justify-content-center">
                            {% csrf_token %}
                            <div class="col-3">
                                <input type="number" name="home_score" class="form-control text-center" value="{{ item.prediction.home_score|default:'' }}" {% if item.is_locked %}disabled{% endif %} required placeholder="casa">
                            </div>
                            <div class="col-auto align-self-center">X</div>
                            <div class="col-3">
                                <input type="number" name="away_score" class="form-control text-center" value="{{ item.prediction.away_score|default:'' }}" {% if item.is_locked %}disabled{% endif %} required placeholder="fora">
                            </div>
            ...
            {% for item in past %}
                <div class="list-group-item d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <small class="text-muted">{{ item.match.date|date:"d/m/Y H:i" }}</small>
                    </div>
                    <div class="text-end">
                        {% if item.prediction %}
